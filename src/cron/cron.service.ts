...
import {
    Game
} from 'src/database/entities/game.entity';
import {
    Team
} from 'src/database/entities/team.entity';
import {
    CreateGameDto
} from 'src/dtos/create-game.dto';
...
export class CronService {
    ...
    async syncGames() {
        ...
        const new_games: any[] = (await Promise.all(data.summaries.map(async (e: { sport_event: { id: any; }; }) => {
            const db = await gameRepository.findOne({ event_id: e.sport_event.id });
            if (!db) return e;
            return false;
        }
        ))).filter(Boolean);
        ...
        new_games.forEach(async game => {
            ...
            dto.team1 = await this.mapTeamID(game.sport_event['competitors'].filter((e) => ["home"].includes(e.qualifier))[0].id);
            dto.team2 = await this.mapTeamID(game.sport_event['competitors'].filter((e) => ["away"].includes(e.qualifier))[0].id);
    ...
    async syncScores() {
        ...
        const data = (await this.httpService.get("https://api.sportradar.com/handball/trial/v2/en/seasons/sr:season:85804/summaries.json?api_key=75wxqg3r57z3cw8acsqfg9fw").toPromise()).data;
        const gameRepository = this.connection.getRepository(Game);
        const new_scores: any[] = (await Promise.all(data.summaries.map(async (e: { sport_event: { id: any; }; }) => {
            const db = await gameRepository.findOne({ event_id: e.sport_event.id, completed: 0 });
            if (db) return e;
            return false;
        }
        ))).filter(Boolean).filter((e: any) => ["closed"].includes(e.sport_event_status.status));

        new_scores.forEach(async score => {
            const game = await gameRepository.findOne({ event_id: score.sport_event.id });
            if (!game) {
                return;
            }
            const update = new UpdateGameDto;
            update.bet = this.getSpecialBet(game.special_bet_id, score.statistics.totals.competitors)
    ...
    getSpecialBet(id: number, data): number {
        switch (id) {
            case 0: // 7m goals
                return data[0].statistics.seven_m_goals + data[1].statistics.seven_m_goals;

            case 1: // yellow cards
                return data[0].statistics.yellow_cards + data[1].statistics.yellow_cards;
            ...
            case 7: // shot accuracy
                return Math.floor((data[0].statistics.shot_accuracy + data[1].statistics.shot_accuracy) / 2);
